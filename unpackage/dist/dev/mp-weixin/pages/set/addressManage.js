...
//
//
var _default = { components: {}, data: function data() {return { addressData: { "addressLabel": "", //地址标签
        "city": "", //城市
        "defaultStatus": 0, //是否默认
        "detailAddress": "", //详细地址
        "id": 0, "memberId": 0, //会员ID
        "name": "", //收货人姓名
        "phoneNumber": "", //电话
        "postCode": "", //邮编
        "province": "", //省份
        "region": "" //区
      }, address: '' };}, onLoad: function onLoad(option) {var title = '添加收货地址';if (option.type === 'edit') {title = '编辑收货地址';this.addressData = JSON.parse(option.data);console.log(this.addressData);}this.manageType = option.type;uni.setNavigationBarTitle({ title: title });}, methods: { switchChange: function switchChange(e) {this.addressData.default = e.detail;}, //地图选择地址
    chooseLocation: function chooseLocation() {var _this = this;uni.getLocation({ type: 'wgs84', success: function success(res) {// console.log('当前位置的经度：' + res.longitude);
          // console.log('当前位置的纬度：' + res.latitude);
        } });uni.chooseLocation({ success: function success(data) {_this.addressData.addressName = data.name;_this.addressData.address = data.name;} });
    },
